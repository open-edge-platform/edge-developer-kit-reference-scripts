...
class ImageClient:
    def __init__(self, host="192.168.123.164", request_port=60000):
        ...
        if self._cam_config.get('table_camera', {}).get('enable_zmq'):
        ...
        if not self._cam_config['forehead_camera']['enable_zmq'] and not self._cam_config['forehead_camera']['enable_webrtc']:
            logger.warning("[Image Client] NOTICE! Head camera is not enabled on both ZMQ and WebRTC.")
        if 'table_camera' in self._cam_config and not self._cam_config['table_camera'].get('enable_zmq') and not self._cam_config['table_camera'].get('enable_webrtc'):
            logger.warning("[Image Client] NOTICE! Table camera is not enabled on both ZMQ and WebRTC.")
    ...
    def get_head_frame(self):
        return self._subscriber_manager.subscribe(self._host, self._cam_config['forehead_camera']['zmq_port'])
    ...
    def get_table_frame(self):
        return self._subscriber_manager.subscribe(self._host, self._cam_config['table_camera']['zmq_port'])
...
def main():
    ...
    running = True
    while running:
        if cam_config['forehead_camera']['enable_zmq']:
            head_img, head_fps = client.get_head_frame()
            if head_img is not None:
                logger.info(f"Head Camera FPS: {head_fps:.2f}")
                logger.debug(f"Head Camera Shape: {cam_config['forehead_camera']['image_shape']}")
                logger.debug(f"Head Camera Binocular: {cam_config['forehead_camera']['binocular']}")
