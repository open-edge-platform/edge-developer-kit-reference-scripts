...
def generate_launch_description() -> LaunchDescription:
    ...
    realsense_config = get_package_share_directory('unitree_a2_nav')+'/config/realsense_a2.yaml'
    ...
    realsense2_camera_launch = IncludeLaunchDescription(
            ...
            launch_arguments={
                ...
                # pointcloud.pointcloud_qos is NOT a declared rs_launch.py arg, so it is
                # silently dropped if passed here. Forward it (and any other raw node
                # params) through the supported 'config_file' mechanism instead.
    ...
    realsense2_camera_tf = Node(
        ...
        # args: x y z yaw pitch roll parent child. pitch=0.7854 rad = 45 deg below horizontal (lens tilted down to ground).
        # arguments=['0.38', '0.0', '0.14', '0.0', '0.5236', '0.0', 'base_link', 'camera_link']
    ...
    output='screen',
    parameters=[{
        'robot_radius': 0.55,          # Unitree A2 footprint radius in meters
        'enable_initial_rotation': False, # Set True to do a 360° scan on startup
    }]
    )
