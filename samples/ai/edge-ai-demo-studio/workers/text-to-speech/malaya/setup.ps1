...
try {
    Write-Host "Starting Malaya setup..." -ForegroundColor Cyan
    Test-UV
    Write-Host "Malaya setup completed successfully!" -ForegroundColor Green
    exit 0
} catch {
    Write-Host "Malaya setup failed: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
} finally {
    Pop-Location
}