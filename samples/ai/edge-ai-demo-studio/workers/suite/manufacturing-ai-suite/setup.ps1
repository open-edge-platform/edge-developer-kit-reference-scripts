...
function Log($msg) { Write-Host "[manufacturing-ai-suite/setup] $msg" }
...
$AppPathDefault = 'manufacturing-ai-suite/industrial-edge-insights-vision'
$ConfigAppPath  = Read-ConfigVar $AppConfig 'EDGE_AI_SUITES_APP_PATH'
$AppPath = if ($env:EDGE_AI_SUITES_APP_PATH) { $env:EDGE_AI_SUITES_APP_PATH } elseif ($ConfigAppPath) { $ConfigAppPath } else { $AppPathDefault }
...
if (Test-Path (Join-Path $SrcDir 'docker-compose.yml')) {
...
try {
  ...
  if (-not (Test-Path $SourcePath)) {
    Log "ERROR: $AppPath not found in repo after sparse-checkout. Verify the path exists in $RepoUrl."
    exit 1
  }
  ...
  Log "Setup complete: $SrcDir (edge-ai-suites ref=$RepoRef, path=$AppPath)"
} finally {
