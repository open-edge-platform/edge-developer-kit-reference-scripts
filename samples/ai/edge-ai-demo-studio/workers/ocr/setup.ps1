...
try {
    Write-Host "Starting OCR setup..." -ForegroundColor Cyan
    ...
    # Summary
    Write-Host ""
    Write-Host "=== OCR Setup Summary ===" -ForegroundColor Cyan
    ...
    if ($FailCount -gt 0) {
        Write-Host "OCR setup completed with $FailCount error(s)." -ForegroundColor Red
        exit 1
    }
    ...
    Write-Host "OCR setup failed: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
} finally {
    Pop-Location
}
