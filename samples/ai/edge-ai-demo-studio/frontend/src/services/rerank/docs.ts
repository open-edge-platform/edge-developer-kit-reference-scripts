...
export const getDocsData = ({
  ...
  const inferModel = model || 'openvino:OpenVINO/bge-reranker-base-int8-ov'
        ...
        params: [
          {
            ...
            type: 'string',
            required: true,
            desc: 'Model identifier in <provider>:<model_id> format, where provider is "openvino" or "llamacpp" (e.g. "openvino:OpenVINO/bge-reranker-base-int8-ov"). Run GET /v1/models to list the available identifiers.',
          },
          {
            name: 'query',
            type: 'string',
...
for result in response.results:
    print(f"Index {result.index}: score={result.relevance_score:.4f}")`,
          },
          {
            ...
            code: `const response = await fetch('${host}/v1/rerank', {
  method: 'POST',
