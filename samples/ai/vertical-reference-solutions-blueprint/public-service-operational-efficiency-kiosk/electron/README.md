...
| `--ask-only` | only ask: prompts on stderr, the resolved build arguments on stdout, one per line — how `./setup.sh --build` asks before its own installs |
...
write. A directory left by an earlier build under the bundle identifier is
renamed on first launch, so upgrading keeps a terminal's data:

```
~/.local/share/VRSB/                        # %APPDATA%\VRSB on Windows
