 ...
 *   node scripts/build.mjs [--bundle-app] --ask-only [args]
 *                                             the questions alone: prompts on
 *                                             stderr, the resolved build args on
 *                                             stdout, one per line — for a
 *                                             launcher that asks before its own
 *                                             long work (./setup.sh --build)
 */
...
// Everything the questions print goes through here: --ask-only moves it to
// stderr so stdout carries only the answers.
let out = process.stdout;
const print = (line = "") => out.write(`${line}\n`);
const step = (message) => print(`\n\x1b[1;36m▸ ${message}\x1b[0m`);
const note = (message) => print(`  \x1b[2m${message}\x1b[0m`);
...
async function bundleApp() {
  ...
  // everything else is bundle.mjs's. The chosen mode decides what it exports
  // (services, deployment profile) and which kiosk the frontend build bakes in.
  const bundleArgs = [...unanswered(argv), "--mode", answers.mode];
  ...
  step("Preparing the shell");
  await run(process.execPath, [path.join(SHELL, "scripts", "make-icons.mjs")], SHELL);
  // The port the export gave the kiosk worker: the shell's staff menu links
  // to the kiosk's pages by it from the platform.
  const bundleEnv = await fs.readFile(path.join(bundleOut, "bundle.env"), "utf8");
  answers.kioskPort = Number(bundleEnv.match(/^KIOSK_BUNDLE_PORT=(\d+)/m)?.[1]) || undefined;
  // Identifies this build's payload: an installed terminal compares it with
  // the stamp on its unpacked platform and refreshes that on a new build.
  answers.bundleId = new Date().toISOString();
  const overlay = await writeShellConfig(answers, { resources: true });
...
/**
 * The questions on their own, for a launcher that wants them answered before
 * its own long work starts. The prompts go to stderr; stdout gets the full
 * argument list for the real build — `--yes`, the answers as flags, then
 * whatever else was passed through — one per line, so the caller can feed it
 * straight back in and the build has nothing left to ask.
 */
async function askOnly() {
  out = process.stderr;
  let answers;
  if (flag("bundle-app")) {
    answers = await askBundleApp();
  } else {
    answers = await ask(await fs.readFile(path.join(FRONTEND, "config.yaml"), "utf8"));
  }
  const resolved = [
    "--yes",
    `--mode=${answers.mode}`,
    answers.fullscreen ? "--fullscreen" : "--windowed",
    `--targets=${answers.targets.join(",")}`,
  ];
  if (!flag("bundle-app")) resolved.push(answers.mock ? "--mock" : "--live");
  process.stdout.write(`${[...resolved, ...unanswered(argv)].join("\n")}\n`);
}

/**
 * The arguments minus the settings the questions decide (in either spelling,
 * `--mode touch` or `--mode=touch`) and the flags that only steer the asking.
 * What is left is the pass-through: bundle.mjs's options, --no-build and co.
 */
function unanswered(args) {
  const decided = new Set(["--yes", "--ask-only", "--bundle-app", "--fullscreen", "--windowed"]);
  if (!flag("bundle-app")) ["--mock", "--live"].forEach((name) => decided.add(name));
  const rest = [];
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === "--mode") { i++; continue; }
    if (decided.has(arg) || arg.startsWith("--mode=") || arg.startsWith("--targets=")) continue;
    rest.push(arg);
  }
  return rest;
}
...
async function choose(rl, question, choices, fallback) {
  ...
  choices.forEach((choice, index) => {
    ...
    print(`  ${marker} ${index + 1}) ${choice.label}`);
  });
  ...
  print("  \x1b[33mnot one of the choices — keeping the default\x1b[0m");
  return fallback;
}
...
async function writeShellConfig(answers, { resources }) {
  const shell = { fullscreen: answers.fullscreen ?? true };
  if (answers.kioskPort) shell.kioskPort = answers.kioskPort;
  if (answers.bundleId) shell.bundleId = answers.bundleId;
  await fs.writeFile(path.join(SHELL, "shell.json"), `${JSON.stringify(shell, null, 2)}\n`);
