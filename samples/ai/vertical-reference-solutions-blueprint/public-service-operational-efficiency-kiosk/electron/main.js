...
/// The platform's own origin when this install has one (the embedded bundle's
/// studio) — the target of F2 and of the staff menu's "Platform" entry. Null
/// for a standalone kiosk, where there is nothing behind the window.
let platformUrl = null;

/// The kiosk's own origin, once the shell knows it — where the staff menu's
/// entries (the registration desk, the settings page) live. Null on the
/// platform when nothing said which port the kiosk took.
let kioskUrl = null;

/// Exit code the kiosk server ends with when it wants to be started again —
/// a settings save at /settings. Next's own restart code, so `next dev` and
/// this shell agree (see frontend/src/app/api/_lib/restart.ts).
const RESTART_EXIT_CODE = 77;

/// Window shape and bundle identity decided at package time
/// (scripts/build.mjs); absent in dev.
let shell = { fullscreen: false };
try { shell = require("./shell.json"); } catch {}

/// Injected into every page the window loads.
///
/// The window has no chrome — no tabs, no back button, no address bar — so
/// the shell adds the navigation a terminal still needs, and keeps it out of
/// a citizen's way:
///
///   - a staff menu (the kiosk, the registration desk, the settings page,
///     the admin, and the platform when there is one), opened with F9 or by
///     tapping the top-right corner five times within three seconds — the
///     pages it leads to are all behind the admin login, this is only the
///     way to reach them without an address bar;
///   - in platform mode, the way back from a sample: F2 (or
///     Ctrl+Shift+Backspace — nothing on screen, a citizen never sees it), and
///     links that ask for a new tab (the gallery's "Open the kiosk") kept in
///     this one window, since the shell has nowhere else to put them.
function shellNavigation() {
  const options = JSON.stringify({
    home: platformUrl,
    homePort: platformUrl ? String(new URL(platformUrl).port || 80) : null,
    kiosk: kioskUrl,
  });
  return `
(function () {
  if (window.__blueprintShell || location.protocol === "file:") return;
  window.__blueprintShell = true;

  var options = ${options};
  var HOME = options.home;
  var onHome = HOME !== null && location.port === options.homePort;
  var kiosk = options.kiosk || (onHome ? null : location.origin);
  ...
  var entries = [];
  if (kiosk) {
    entries.push(["Kiosk", kiosk + "/"]);
    entries.push(["Registration desk", kiosk + "/enroll"]);
    entries.push(["Kiosk settings", kiosk + "/settings"]);
    entries.push(["Admin (CMS)", kiosk + "/admin"]);
  }
  if (HOME !== null && !onHome) entries.push(["Platform", HOME]);
  ...
  var openMenu = function () {
    ...
    card.setAttribute("style", [
      ...
      "padding:16px", "border-radius:16px", "background:#14161c", "color:#fff",
      "box-shadow:0 20px 60px rgba(0,0,0,.5)",
    ].join(";"));
    var title = document.createElement("div");
    title.textContent = "Staff";
    title.setAttribute("style", "font-weight:600;font-size:12px;opacity:.6;letter-spacing:.08em;text-transform:uppercase;margin-bottom:4px");
    ...
    var item = function (label, action) {
      ...
      button.setAttribute("style", [
        ...
        "font:500 14px system-ui,-apple-system,sans-serif", "text-align:left",
      ].join(";"));
      button.addEventListener("click", action);
      card.appendChild(button);
    };
    entries.forEach(function (entry) {
      item(entry[0], function () { location.href = entry[1]; });
    });
  ...
  var toggleMenu = function () { if (menu) closeMenu(); else openMenu(); };

  document.addEventListener("keydown", function (event) {
    if (event.key === "Escape" && menu) { closeMenu(); return; }
    if (event.key === "F9") { event.preventDefault(); toggleMenu(); return; }
    if (HOME !== null && !onHome &&
        (event.key === "F2" || (event.ctrlKey && event.shiftKey && event.key === "Backspace"))) {
      location.href = HOME;
    }
  });

  // Five taps in the top-right corner within three seconds.
  var taps = [];
  document.addEventListener("pointerdown", function (event) {
    var corner = event.clientX > window.innerWidth - 96 && event.clientY < 96;
    if (!corner) { taps = []; return; }
    var now = Date.now();
    taps = taps.filter(function (t) { return now - t < 3000; });
    taps.push(now);
    if (taps.length >= 5) { taps = []; toggleMenu(); }
  }, true);
})();
`;
}
...
/// config edits and unpacked studio. The short "VRSB" name keeps the deeply
/// nested unpacked studio under MAX_PATH; a directory left by an earlier build
/// under the bundle identifier is renamed on first launch.
function checkedPath(raw) {
  ...
  if (out.includes("..")) throw new Error("the data directory path contains a traversal segment");
  return out;
}
...
function dataDir() {
  ...
  const dir = path.join(base, "VRSB");
  const legacy = path.join(base, "com.verticalreferencesolutionsblueprint.desktop");
  if (!fs.existsSync(dir) && fs.existsSync(legacy)) {
    fs.renameSync(checkedPath(legacy), checkedPath(dir));
  }
  return dir;
}
...
function createWindow() {
  ...
  win.webContents.on("did-finish-load", () => {
    win.webContents.executeJavaScript(shellNavigation()).catch(() => {});
  });
...
///
/// windowsHide: the shell is a GUI process with no console of its own, so a
/// console child (cmd.exe, tar) would allocate — and show — a new one, and the
/// whole studio stack under it would run behind that black window. The flag
/// gives the child an invisible console the rest of the tree inherits, which is
/// what the studio's own launcher achieves by staying in one inherited console
/// (`Start-Process -NoNewWindow`). It is ignored when `detached` is set, hence
/// detached staying false on Windows.
...
///
/// A server that exits with RESTART_EXIT_CODE asked for it (a settings save)
/// and is started again on the same port; any other exit is left alone.
...
/// Unpack (first launch, or a new build), set up (same) and start the
/// embedded bundle, then point the window at the kiosk once it answers.
///
/// The kiosk is the terminal's whole reason to exist, so it is what the window
/// opens on; the studio that runs it stays one staff-menu entry away. A kiosk
/// that never answers falls back to the studio, where its worker's log is.
async function runBundle(win, tar, data) {
  ...
  const kioskPort = Number(shell.kioskPort) || 8035;
  if (await waitForPort(kioskPort, timeoutMs)) {
    navigate(win, kioskUrl);
  } else if (await waitForPort(STUDIO_PORT, 1000)) {
    splashNote(win, "The kiosk did not start - opening the platform...");
    navigate(win, platformUrl);
  } else {
    showFailed(win);
  }
}
...
async function prepareBundle(win, tar, data, studio) {
  ...
  // The unpacked tree is stamped with the build it came from. A package of
  // a newer build finds an older stamp (or none — a tree from before stamps
  // existed) and replaces the tree: the platform's source and the kiosk
  // server are the build's, and stale copies would keep serving old pages.
  // What the terminal itself owns is carried across: the kiosk worker's
  // data (its config.yaml, database, documents, portraits) and the runtimes
  // the setup downloaded (thirdparty/, skipped by the setup when present).
  // The worker environments are made again by the setup that follows.
  const stampFile = path.join(target, ".bundle-id");
  const installed = fs.existsSync(stampFile) ? fs.readFileSync(stampFile, "utf8").trim() : "";
  const stale = fs.existsSync(target) && shell.bundleId && installed !== shell.bundleId;
  const previous = path.join(dir, "kiosk-studio.previous");
  if (stale) {
    splashNote(win, "Updating the platform to this build...");
    fs.rmSync(previous, { recursive: true, force: true });
    fs.renameSync(target, previous);
  }

  if (!fs.existsSync(target)) {
    if (!stale) splashNote(win, "Unpacking the platform (first launch)...");
    ...
    fs.rmSync(scratch, { recursive: true, force: true });
    if (shell.bundleId) fs.writeFileSync(stampFile, `${shell.bundleId}\n`);
  }

  if (fs.existsSync(previous)) {
    const worker = path.join("workers", "public-service-kiosk", "data");
    if (fs.existsSync(path.join(previous, worker))) {
      fs.rmSync(path.join(target, worker), { recursive: true, force: true });
      fs.mkdirSync(path.dirname(path.join(target, worker)), { recursive: true });
      fs.renameSync(path.join(previous, worker), path.join(target, worker));
    }
    if (fs.existsSync(path.join(previous, "thirdparty")) && !fs.existsSync(path.join(target, "thirdparty"))) {
      fs.renameSync(path.join(previous, "thirdparty"), path.join(target, "thirdparty"));
    }
    fs.rmSync(previous, { recursive: true, force: true });
  }
...
app.whenReady().then(async () => {
  ...
  if (url) {
    ...
    // Whatever the window opens on, the staff menu still needs both origins:
    // the kiosk it serves and the platform behind it, when there is one.
    kioskUrl = (process.env.KIOSK_SHELL_KIOSK_URL || (port === STUDIO_PORT ? "" : url)).replace(/\/+$/, "") || null;
    platformUrl =
      (process.env.KIOSK_SHELL_PLATFORM_URL || (port === STUDIO_PORT ? url : "")).replace(/\/+$/, "") || null;
    ...
    if (cmd) {
      ...
      if (process.env.KIOSK_SHELL_KEEP_ALIVE) {
        spawn(runner[0], runner.slice(1), { ...options, stdio: "inherit", windowsHide: true });
      } else {
    ...
    if (await waitForPort(port, timeoutMs)) {
      navigate(win, url);
    } else if (platformUrl && platformUrl !== url &&
               (await waitForPort(Number(new URL(platformUrl).port) || 80, 1000))) {
      // The kiosk never answered but the stack behind it is up — open that
      // rather than a dead end; its logs are the next thing anyone wants.
      splashNote(win, "The kiosk did not start - opening the platform...");
      navigate(win, platformUrl);
    } else {
      showFailed(win);
  ...
  // up as its own worker process; the window opens on the kiosk.
  const bundleTar = bundled(resources, "kiosk-studio.tar");
  if (fs.existsSync(bundleTar)) {
    platformUrl = `http://127.0.0.1:${STUDIO_PORT}`;
    kioskUrl = `http://127.0.0.1:${Number(shell.kioskPort) || 8035}`;
