...
# The package identity lives in the shell's package.json (the identifier in
# its build config); the defaults are the fallback for a copy of this script
# that travels without the checkout.
PKG_JSON="$ELECTRON_DIR/package.json"
pkg_value() {
  [ -f "$PKG_JSON" ] || return 1
  sed -n "s/^[[:space:]]*\"$1\"[[:space:]]*:[[:space:]]*\"\([^\"]*\)\".*/\1/p" "$PKG_JSON" | head -1
}
PRODUCT="$(pkg_value productName || true)"
BINARY="$(pkg_value name || true)"
PRODUCT="${PRODUCT:-Vertical Reference Solutions Blueprint}"
IDENTIFIER="com.verticalreferencesolutionsblueprint.desktop"
BINARY="${BINARY:-kiosk-desktop}"
# Data lives under the short "VRSB" name; earlier builds used the full identifier.
DATA_BASE="${XDG_DATA_HOME:-$HOME/.local/share}"
DATA_DIR="$DATA_BASE/VRSB"
[ -d "$DATA_DIR" ] || [ ! -d "$DATA_BASE/$IDENTIFIER" ] || DATA_DIR="$DATA_BASE/$IDENTIFIER"
