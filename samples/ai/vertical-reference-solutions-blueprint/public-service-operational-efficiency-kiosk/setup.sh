...
#
# Every question — setup's own and the build's (kiosk mode, window, package
# format) — is asked up front; the installs and the build run unattended after.
#
...
Questions come first: whatever setup has to ask (the studio's system packages)
and the build's install settings (kiosk mode, window, package format) are
answered before the first npm install — after that, everything runs unattended.
...
# The studio's install_dependencies.sh: --yes runs it with -y.
install_studio_deps() {
  if [ "$YES" -eq 1 ]; then
...
# setup, the build), so whatever needs an answer is asked here.
STUDIO_DEPS=0
if [ "$STUDIO" -eq 1 ] || [ "$BUNDLE" -eq 1 ]; then
...
if [ "$BUILD" -eq 1 ]; then
  if [ "$YES" -eq 1 ]; then
    BUILD_ARGS=(--yes ${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"})
  elif [ -t 0 ]; then
    # The build's own questions (kiosk mode, window, package format), asked by
    # the build script now rather than after the installs. It prints the
    # complete argument list back — --yes, the answers as flags, then the
    # pass-through — so the build later has nothing left to ask.
    mapfile -t BUILD_ARGS < <(node "$ELECTRON_DIR/scripts/build.mjs" --bundle-app --ask-only ${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"})
    [ "${#BUILD_ARGS[@]}" -gt 0 ] || die "the build's questions were not answered"
  fi
fi
...
if [ "$BUNDLE" -eq 1 ]; then
  ...
  [ "$STUDIO_DEPS" -eq 0 ] || install_studio_deps
