...
└── app/
    ...
    ├── settings/                    Settings page (staff): edits config.yaml, restarts the kiosk
...
- **Settings page** — [http://localhost:3000/settings](http://localhost:3000/settings),
  linked from the admin nav. This terminal's `config.yaml` as a form (and as
  the file itself), with a restart button that makes a save count — see
  [docs/configuration.md](../docs/configuration.md#the-settings-page).
