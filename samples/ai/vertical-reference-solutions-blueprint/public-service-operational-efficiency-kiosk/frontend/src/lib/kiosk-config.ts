 ...
 * is never assigned, so "unset" keeps its meaning. Node only — the browser
 * half reads the same values through src/lib/public-config.ts, served with
 * the page rather than inlined at build time.
 */
...
const SETTINGS = {
  ...
  camera: { timeout_ms: "NEXT_PUBLIC_KIOSK_CAMERA_TIMEOUT_MS" },
  nfc: {
...
/** Where this terminal's settings live: the selected file (or config.yaml,
 *  or the committed reference profile until a launcher has copied one out)
 *  and the machine-local override layered on top of it. `fallback` is true
 *  when `main` is that committed stand-in — a file nothing should write to. */
export function kioskConfigFiles(root = process.cwd()): {
  main: string;
  local: string;
  fallback: boolean;
} {
  ...
  const fallback = !selected && !existsSync(primary);
  const main = fallback ? path.join(root, "configs", "reference.yaml") : primary;
  ...
  return { main, local, fallback };
}

/** The env var a dotted YAML key maps to, or undefined for a key the kiosk
 *  does not know. */
export function settingEnv(key: string): string | undefined {
  let node: string | Tree | undefined = SETTINGS;
...
/** Every browser-visible env var the kiosk knows, for the layout to serve
 *  with the page — see src/components/kiosk-env.tsx. */
export function publicSettingEnvNames(): string[] {
  const out: string[] = [];
  const walk = (node: Tree) => {
    for (const value of Object.values(node)) {
      if (typeof value === "string") {
        if (value.startsWith("NEXT_PUBLIC_")) out.push(value);
      } else {
        walk(value);
      }
    }
...
/** One parsed settings tree as the flat env-var map, plus the keys it
 *  carries that the kiosk does not know. */
export function flattenKioskConfig(config: Plain): {
...
/** Parse settings YAML text the way the loader would. Throws on YAML that
 *  does not parse or is not a mapping. */
export function parseKioskConfigText(text: string): Plain {
  const parsed: unknown = parse(text);
  if (parsed == null) return {};
  if (!isPlainObject(parsed)) throw new Error("config.yaml must be a mapping of settings.");
  return parsed;
}

export function readKioskConfig(root = process.cwd()): Record<string, string> {
  const { main, local } = kioskConfigFiles(root);
  const config = merge(readYaml(main), readYaml(local));
  const { values, unknown } = flattenKioskConfig(config);
...
/** The browser-visible subset. Inlined by `next.config.ts` as a fallback for
 *  anything rendered outside the kiosk layout; the kiosk itself is served the
 *  live values — see src/lib/public-config.ts. */
