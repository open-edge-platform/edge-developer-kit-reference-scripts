...
import { documentSource, getMockDocument, waitForScannedDocument } from "@/lib/api/kiosk";
import { matchOptionAnswers } from "@/lib/ask-match";
import { idGesture, idReaderCopy } from "@/lib/id-reader";
import { publicBoolean, publicEnv } from "@/lib/public-config";
...
const waitForSpeech = () => publicBoolean("NEXT_PUBLIC_KIOSK_WAIT_FOR_SPEECH", true);
...
const defaultLanguage = () =>
  publicEnv("NEXT_PUBLIC_KIOSK_LANG") ?? activePack().locale.language;
...
> => ({
  insertDocument: {
    ...
    confirm: idReaderCopy().confirm,
    help: t("deviceStep.insertDocument.help", { where: idReaderCopy().where }),
  },
  biometric: {
...
export function ChatKiosk({ api }: { api?: string } = {}) {
  ...
  const labelFor = (ask: Ask, value: string) =>
    ask.type === "action" && value === "done"
      ? (deviceSteps()[ask.id]?.receipt ?? "Done at the kiosk")
  ...
  const narrating = waitForSpeech() && speaking;
  ...
  const autoCapture = documentSource() !== "upload";
...
function AskRow({
  ...
  const scanning = ask.type === "document" && documentSource() === "scanner" && !manual;
    ...
    <div className="flex flex-col items-center gap-7 text-center sm:flex-row sm:items-center sm:gap-9 sm:text-left">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-4 flex flex-col items-center gap-3 sm:items-start">
          ...
          {spoke && !captured && (
            <StepNag help={deviceSteps().biometric.help}>
              <Button
                className="ks-attention-ring h-13 rounded-2xl px-6 text-lg font-semibold"
                onClick={() => {
                  setCapturedAt(turn);
...
function stepReceipt(message: KioskUIMessage): string | null {
  ...
  const ids = Object.keys(answers).filter((id) => id in steps && answers[id] === "done");
  if (ids.length === 0) return null;
  return ids.map((id) => steps[id].receipt).join(" · ");
}
