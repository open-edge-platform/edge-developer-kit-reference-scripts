...
/**
 * The region of the stream that `object-cover` actually puts on screen, in
 * stream pixels — so the picture matched is the picture the citizen framed
 * themselves in, not the wider frame the preview cut away. Null until the
 * stream has a frame; the whole frame when the element has no layout box.
 */
export function visibleCrop(
  ...
  if (videoWidth / videoHeight > boxRatio) {
    const width = Math.round(videoHeight * boxRatio);
    return { x: Math.round((videoWidth - width) / 2), y: 0, width, height: videoHeight };
  }
  const height = Math.round(videoWidth / boxRatio);
  return { x: 0, y: Math.round((videoHeight - height) / 2), width: videoWidth, height };
}
