...
import { publicNumber } from "@/lib/public-config";
...
const restartSeconds = () =>
  Math.max(1, Math.round(publicNumber("NEXT_PUBLIC_KIOSK_RESTART_MS", 30_000) / 1000));

/** How long a session may sit untouched before the kiosk clears it down. */
const idleMs = () => publicNumber("NEXT_PUBLIC_KIOSK_IDLE_MS", 60_000);
...
export function useAutoRestart({
  ...
  const fresh = { armed: when, secondsLeft: restartSeconds(), held: false };
 ...
 * conversation) for `session.idle_ms`. The clock is held while the
...
export function useIdleRestart({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => idleRef.current(), idleMs());
    return () => {
      clearTimeout(timer);
