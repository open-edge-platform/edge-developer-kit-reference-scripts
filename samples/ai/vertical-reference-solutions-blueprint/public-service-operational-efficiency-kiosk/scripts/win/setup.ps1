...
.DESCRIPTION
  ...
  through setup_win.bat. Every question is asked up front - the build's
  install settings (kiosk mode, window, package format) included - and the
  installs and the build run unattended after.
...
Questions come first: the build's install settings (kiosk mode, window, package
format) are answered before the first npm install - after that, everything runs
unattended.
...
# 0. Questions ---------------------------------------------------------------
# Everything below this block is unattended work (npm installs, the studio's
# setup, the build), so whatever needs an answer is asked here. Setup itself
# has nothing to ask on Windows (the studio's system-package step is Linux
# only); the build's questions are the build script's own.
if ($Build) {
    if ($Yes) {
        $BuildArgs = @('--yes') + $BuildArgs
    } elseif (-not [Console]::IsInputRedirected) {
        # The build's own questions (kiosk mode, window, package format), asked
        # by the build script now rather than after the installs. It prints the
        # complete argument list back - --yes, the answers as flags, then the
        # pass-through - so the build later has nothing left to ask.
        $asker = Join-Path $ElectronDir 'scripts\build.mjs'
        $BuildArgs = @(& node $asker --bundle-app --ask-only @BuildArgs)
        if ($LASTEXITCODE -ne 0) { Stop-WithError "the build's questions failed (exit $LASTEXITCODE)" }
        if ($BuildArgs.Count -eq 0) { Stop-WithError "the build's questions were not answered" }
    }
}
