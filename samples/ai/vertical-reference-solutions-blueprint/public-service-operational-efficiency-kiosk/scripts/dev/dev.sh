...
#   scripts/dev/dev.sh --desktop  run the desktop shell in dev mode instead
#
...
Options:
  ...
  --desktop      run the desktop (Electron) shell in dev mode instead of the
                 ...
                 script (e.g. scripts/dev/dev.sh --desktop -- --yes --mode=touch)
  -h, --help     this help
...
if [ "$DESKTOP" -eq 1 ]; then
  info "Starting the desktop shell in dev mode (electron/)"
  cd "$ELECTRON_DIR" || die "cannot enter $ELECTRON_DIR"
