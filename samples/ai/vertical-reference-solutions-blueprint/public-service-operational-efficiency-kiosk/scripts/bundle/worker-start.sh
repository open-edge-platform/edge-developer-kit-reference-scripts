...
# Vertical Reference Solutions Blueprint as an Edge AI Studio worker.
#
...
#
# Mirrors what the kiosk's desktop shell does (electron/main.js):
...
# Exit code 77 is the kiosk asking to be started again (a settings save at
# /settings). Looping here keeps this PID — the one the studio watches — alive.
export KIOSK_SUPERVISED=1
while :; do
  code=0
  "$NODE" kiosk.cjs || code=$?
  [ "$code" -eq 77 ] || exit "$code"
done
