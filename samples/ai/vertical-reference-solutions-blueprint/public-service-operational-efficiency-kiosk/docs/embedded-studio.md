...
| http://localhost:8035/enroll | Staff registration desk (CMS admin login) |
| http://localhost:8035/settings | Settings page — edits the worker's `data/config.yaml`, restarts the kiosk worker in place (CMS admin login) |
...
the launcher sets for you: `KIOSK_SHELL_URL`, `KIOSK_SHELL_PLATFORM_URL` (the
studio, one staff-menu entry behind the kiosk), `KIOSK_SHELL_CMD`,
...
| `--mode` | Services auto-started | Kiosk `terminal.mode` shipped as |
|---|---|---|
| `touch` | OCR (NPU), face (CPU) — no speech, and no local LLM: the terminal calls a remote text-generation gateway | touch terminal |
| `chat` / `agent` | all five, locally: LLM (GPU), STT (NPU), OCR, face, TTS (CPU) | chat / agent terminal |
...
- The kiosk's own settings are *not* frozen into the staged build: its
  `config.yaml` lives in the worker's `data/` directory and every key there —
  terminal mode and document source included — takes effect on the next
  restart (see
