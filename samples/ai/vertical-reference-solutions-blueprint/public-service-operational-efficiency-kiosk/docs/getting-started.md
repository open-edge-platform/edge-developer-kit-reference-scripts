...
Whatever the run has to ask — setup's own question about the studio's system
packages, the build's kiosk mode / window / package format — comes before the
first install; after that it runs unattended.
...
| http://localhost:3000/settings | Settings page — edits `config.yaml`, restarts the kiosk to apply (same login as `/admin`) |
