...
| **Edge AI Studio** checkout | The AI gateway providing LLM, OCR, face, STT and TTS. Default location `~/workspace/applications.ai.tools.edge-ai-studio`; configurable via `EDGE_AI_STUDIO_DIR`. Optional if you run with `--mock`. |
| Linux (Debian/Ubuntu assumed) or Windows | Each launcher ships twice: `setup.sh`/`start.sh`/`build.sh` for bash, and `setup_win.bat`/`start_win.bat`/`build_win.bat` for Windows (the `_win` suffix keeps them distinguishable when Explorer hides extensions), which run [scripts/win/](../scripts/win/)'s PowerShell equivalents with `-ExecutionPolicy Bypass`. Flags are spelled the same. The embedded-bundle build is Linux/WSL only. |
...
This installs npm dependencies for [frontend/](../frontend/) and [tauri/](../tauri/),
then sets up the **Edge AI Studio** prerequisite from `EDGE_AI_STUDIO_DIR`
(its system dependencies via sudo, its bundled runtimes, AI workers, and gateway build —
the studio step is long on first run since it downloads models and toolchains).

Useful variants:

```bash
./setup.sh --yes                  # non-interactive (CI / provisioning)
./setup.sh --skip-studio          # kiosk only; you'll run with --mock or a remote gateway
./setup.sh --package-studio       # also build the studio's distributable executable
./setup.sh --desktop              # also install the Rust/WebKitGTK toolchain for desktop bundles
./setup.sh --hardware             # also install pcscd (NFC), sane-utils (scanner), poppler-utils (OCR)
```

On Windows the same flags reach `scripts\win\setup.ps1`. Two behave differently
there, because there is nothing to apt-install: `--desktop` checks for Rust and
the MSVC C++ build tools and prints the `winget` lines for whatever is missing,
and `--hardware` prints the vendor drivers to install by hand (Smart Card
service, PaperStream IP for the fi-800R, poppler on PATH).
...
```bash
./start.sh --tauri       # launch the packaged desktop app (still auto-starts the studio)
./start.sh --rebuild     # force a fresh production build first
./start.sh --port 4000   # serve on another port
./start.sh --no-studio   # gateway managed elsewhere (set STUDIO_URL accordingly)
...
`--tauri` launches the desktop shell built by `./build.sh` (usually combined
with `--bundle`). The studio is started first, exactly as for the web server.
...
2. **Ensures the Edge AI Studio gateway is up** at `STUDIO_URL` (default
   `http://localhost:8080`). If it isn't, it launches it from `EDGE_AI_STUDIO_DIR` —
   preferring the packaged executable under `out/EdgeAIDemoStudio/` if you built one,
   falling back to the studio's headless `start.sh` — and waits until the gateway
   answers (`STUDIO_WAIT_SECS`, default 600 s; the first launch loads AI models).
   It then probes the five services the kiosk uses and warns about any that aren't
   active.
...
The launcher scripts handle this for you: before starting the studio they install a
preset file as the studio's `deployment.json`, chosen by the kiosk's **terminal
mode** (from `NEXT_PUBLIC_KIOSK_MODE` / `config.local.yaml` / `config.yaml`):

| Kiosk mode | Profile installed | Services auto-started |
|---|---|---|
| `touch` | [scripts/studio-deployment.touch.json](../scripts/studio-deployment.touch.json) | **text-generation + ocr** only (face/STT/TTS explicitly offline) |
| `chat` / `agent` | [scripts/studio-deployment.chat.json](../scripts/studio-deployment.chat.json) | all five services |

Both profiles hardcode the devices (LLM on `GPU`, everything else on `CPU` — edit the
files to retarget) and pin text-generation to the model the kiosk's `config.yaml`
expects (`openvino:OpenVINO/Qwen3.5-4B-int4-ov`). A preset file these scripts
installed is upgraded in place when you switch modes; a `deployment.json` you wrote
by hand is never overwritten (a `.bak` is kept whenever a file is replaced). Disable
the behaviour with `STUDIO_DEPLOYMENT_MANAGE=0`, or force a specific file with
