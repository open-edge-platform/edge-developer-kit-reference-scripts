...
# Shared helpers for the vertical-reference-blueprint launcher scripts (setup/start/dev/build).
...
EDGE_AI_STUDIO_DIR="${EDGE_AI_STUDIO_DIR:-$HOME/workspace/applications.ai.tools.edge-ai-studio}"
...
#   touch      -> scripts/studio-deployment.touch.json  (LLM, OCR, face — no speech)
...
# The file is read on every studio boot. A deployment.json these scripts
# installed earlier is upgraded in place (tracked via .studio-deployment.installed,
# so switching kiosk modes re-installs the right profile); one written by hand
# is never overwritten.
studio_ensure_deployment() {
  [ "$STUDIO_DEPLOYMENT_MANAGE" = "1" ] || return 0
  studio_present || return 0
  local file target marker
  ...
  marker="$REPO_ROOT/.studio-deployment.installed"

  if [ -f "$target" ] && grep -q '"status"' "$target"; then
    ...
    if ! { [ -f "$marker" ] && cmp -s "$target" "$marker"; }; then
      info "Studio deployment.json has its own presets — leaving it as is"
      info "(replace it manually with $file if the kiosk services don't auto-start)"
      return 0
    fi
  fi
...
Set EDGE_AI_STUDIO_DIR (env var or .kioskrc) to your checkout, or pass --no-studio / --mock."
...
# The packaged desktop app: newest .AppImage from the Tauri release bundle,
# else the plain release binary. Non-zero if neither exists.
desktop_app_path() {
  local app="" f
  for f in "$TAURI_DIR"/src-tauri/target/release/bundle/appimage/*.AppImage; do
    [ -f "$f" ] || continue
    if [ -z "$app" ] || [ "$f" -nt "$app" ]; then app="$f"; fi
  done
  if [ -z "$app" ] && [ -x "$TAURI_DIR/src-tauri/target/release/kiosk-desktop" ]; then
    app="$TAURI_DIR/src-tauri/target/release/kiosk-desktop"
  fi
  [ -n "$app" ] || return 1
  printf '%s\n' "$app"
}
