...
$Product = 'Vertical Reference Blueprint'
$Identifier = 'com.verticalreferenceblueprint.desktop'
$Binary = 'kiosk-desktop'
$tauriConf = Join-Path $TauriDir 'src-tauri\tauri.conf.json'
if (Test-Path -LiteralPath $tauriConf) {
    $conf = Get-Content -LiteralPath $tauriConf -Raw | ConvertFrom-Json
    if ($conf.productName) { $Product = $conf.productName }
    if ($conf.identifier) { $Identifier = $conf.identifier }
}
$cargoToml = Join-Path $TauriDir 'src-tauri\Cargo.toml'
if (Test-Path -LiteralPath $cargoToml) {
    $match = Select-String -LiteralPath $cargoToml -Pattern '^name\s*=\s*"([^"]+)"' | Select-Object -First 1
    if ($match) { $Binary = $match.Matches[0].Groups[1].Value }
}
