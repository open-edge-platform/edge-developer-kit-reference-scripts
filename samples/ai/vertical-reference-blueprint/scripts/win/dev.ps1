...
$Mock = $false; $NoStudio = $false; $Tauri = $false
$ExtraArgs = @()
...
Options:
  ...
  --tauri        run the desktop (Tauri) shell in dev mode instead of the
                 ...
                 script (e.g. scripts\dev\dev.bat --tauri -- --yes --mode=touch)
  -h, --help     this help
...
if ($Tauri) {
    Write-Info 'Starting the desktop shell in dev mode (tauri/)'
    Invoke-Npm -NpmArgs (@('run', 'dev', '--') + $ExtraArgs) -WorkingDirectory $TauriDir
    exit 0
}
