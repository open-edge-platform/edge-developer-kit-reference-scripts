...
<#
.SYNOPSIS
  Shared helpers for the vertical-reference-blueprint launcher scripts on Windows.
...
$EdgeAiStudioDir = Get-Setting 'EDGE_AI_STUDIO_DIR' (Join-Path $HOME 'workspace\applications.ai.tools.edge-ai-studio')
...
# then config.yaml, then the touch fallback. The `mode:` read here is the one
# inside the top-level `terminal:` block only (the scanner block has its own);
# frontend/src/lib/kiosk-config.ts is the source of truth for that shape.
function Get-KioskTerminalMode {
    ...
    foreach ($name in @('config.local.yaml', 'config.yaml')) {
        $file = Join-Path $FrontendDir $name
...
# services the kiosk needs auto-start with the model/devices it expects. A
# deployment.json these scripts installed earlier is upgraded in place (tracked
# via .studio-deployment.installed); one written by hand is never overwritten.
function Install-StudioDeployment {
    ...
    $marker = Join-Path $RepoRoot '.studio-deployment.installed'
    ...
    if ((Test-Path -LiteralPath $target) -and (Select-String -LiteralPath $target -Pattern '"status"' -Quiet)) {
        if (& $sameAs $target $file) {
            Write-Info "Studio deployment.json already matches the $(Split-Path $file -Leaf) profile"
            return
        }
        if (-not (& $sameAs $target $marker)) {
            Write-Info 'Studio deployment.json has its own presets - leaving it as is'
            Write-Info "(replace it manually with $file if the kiosk services don't auto-start)"
            return
        }
        Copy-Item -LiteralPath $target -Destination "$target.bak" -Force
    }
    Copy-Item -LiteralPath $file -Destination $target -Force
    Copy-Item -LiteralPath $file -Destination $marker -Force
...
Set EDGE_AI_STUDIO_DIR (env var or .kioskrc) to your checkout, or pass -Mock / -NoStudio.
"@
    }
...
function Test-StudioServices {
    ...
    if ($mode -ne 'touch') {
        $checks['face-recognition'] = "$StudioUrl/api/face-recognition/healthcheck"
