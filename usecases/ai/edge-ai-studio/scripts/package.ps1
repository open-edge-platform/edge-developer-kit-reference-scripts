...
function Invoke-FinalizePackage {
    ...
    Write-Host "Creating EdgeAIStudio package structure..." -ForegroundColor Green
    $outDir = Join-Path $SCRIPT_DIR "../out"
    Push-Location $outDir
    
    try {
        # Remove existing EdgeAIStudio directory if it exists
        if (Test-Path "EdgeAIStudio") {
            Remove-Item -Recurse -Force "EdgeAIStudio"
            Write-Host "Removed existing EdgeAIStudio directory" -ForegroundColor Green
        }
        
        # Create EdgeAIStudio directory
        New-Item -ItemType Directory -Path "EdgeAIStudio" | Out-Null
        
        # Copy files to EdgeAIStudio root
        $rootFiles = @("README.md", "install_dependencies.sh", "run_web.ps1", "setup.ps1")
        foreach ($rootFile in $rootFiles) {
            $sourcePath = Join-Path $outDir $rootFile
            if (Test-Path $sourcePath) {
                Copy-Item $sourcePath "EdgeAIStudio/" -Force
                Write-Host "$rootFile copied to EdgeAIStudio root successfully." -ForegroundColor Green
            }
        }
        
        # Copy the entire win-unpacked directory into EdgeAIStudio
        if (Test-Path "win-unpacked") {
            Copy-Item -Recurse "win-unpacked" "EdgeAIStudio/" -Force
...
set "EXECUTABLE=%SCRIPT_DIR%win-unpacked\EdgeAIStudio.exe"
...
'@
        ...
        $launcherContent | Out-File -FilePath "EdgeAiStudio.bat" -Encoding ASCII
        Write-Host "EdgeAiStudio.bat launcher script created successfully." -ForegroundColor Green
        Pop-Location
        
        # Create zip file with the new structure
        Write-Host "Creating EdgeAIStudio.zip..." -ForegroundColor Green
        if (Test-Path "EdgeAIStudio.zip") {
            Remove-Item "EdgeAIStudio.zip" -Force
            Write-Host "Removed existing EdgeAIStudio.zip" -ForegroundColor Green
        }
        ...
        Normalize-Timestamps -RootPath "EdgeAIStudio"

        # Use PowerShell's Compress-Archive cmdlet
        Compress-Archive -Path "EdgeAIStudio" -DestinationPath "EdgeAIStudio.zip" -Force
        Write-Host "EdgeAIStudio.zip created successfully." -ForegroundColor Green
