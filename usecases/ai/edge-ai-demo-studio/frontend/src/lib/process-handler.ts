  ...
  return new Promise((resolve) => {
    ...
    if (process.platform === 'win32') {
      ...
      exec(`taskkill /PID ${proc.pid} /T /F`, (err) => {
        if (err) {
          logger.error(`Failed to kill process tree for ${name}:`, err)
          proc.kill('SIGKILL')
        }
      })
    } else {
...
async function clearProcessLogs(name: string) {
  ...
  try {
    ...
    logger.error(`Failed to clear log file ${logFile}:`, error)
    return false
  }
}
...
function removeDeadProcess(name: string): boolean {
  logger.log('Removing dead process from list:', name)
  const entry = processes.get(name)
  if (!entry) return false
