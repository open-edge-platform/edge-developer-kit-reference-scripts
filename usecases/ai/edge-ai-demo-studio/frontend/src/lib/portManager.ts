...
export async function checkAndHandlePortConflicts(): Promise<{
  ...
  if (externalProcesses.length > 0) {
    ...
    console.warn('The following ports are in use by external processes:\n')

    externalProcesses.forEach((conflict) => {
      console.warn(`  Port ${conflict.port}:`)
      console.warn(`    PID: ${conflict.processInfo?.pid}`)
      console.warn(`    Process: ${conflict.processInfo?.processName}`)
      if (conflict.processInfo?.commandLine) {
        console.warn(`    Command: ${conflict.processInfo.commandLine}`)
      }
      console.warn('')
    })

    console.warn('⚠️  WARNING: External processes are using required ports.')
    console.warn('This may cause service failures when starting workloads.\n')
    console.warn('To kill these processes manually:')

    if (isWindows) {
      externalProcesses.forEach((conflict) => {
        console.warn(`  taskkill /PID ${conflict.processInfo?.pid} /F`)
      })
    } else {
      externalProcesses.forEach((conflict) => {
        console.warn(`  kill -9 ${conflict.processInfo?.pid}`)
      })
    }
