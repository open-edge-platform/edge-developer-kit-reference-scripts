...
import type { ApiLogEntry, ApiLogResponse } from '@/types/common'
...
function parseLogLines(lines: string[]): ApiLogEntry[] {
  const parsed: ApiLogEntry[] = []
  for (const line of lines) {
    try {
      ...
      parsed.push({
        ...
        level: logEntry.level ?? logEntry.type,
      })
    } catch {}
  }
  return parsed
...
async function getFilteredLogs(
  ...
  readSize: number,
  sinceTime: number,
): Promise<{ logs: ApiLogEntry[]; actualReadSize: number }> {
  const content = await readFileContent(logFile, readSize, offset)
  const lines = content.split('\n').filter(Boolean)
  const filtered: ApiLogEntry[] = []
...
export async function GET(
  req: NextRequest,
): Promise<NextResponse<ApiLogResponse | { error: string }>> {
  ...
  const logFile =
    source === 'old'
      ? path.join(LOGS_DIR, 'old', `${name}.log`)
      : path.join(LOGS_DIR, `${name}.log`)
