...
const BASE_URL = '/api/ppt-translator'

function useHealthCheck(enabled: boolean) {
  return useQuery({
    queryKey: ['ppt-translator-health'],
    queryFn: async () => {
...
export function PptTranslatorDemo({ service }: { service: Service }) {
    ...
    <div className="flex flex-col items-center justify-center gap-6 py-16">
      ...
      <div className="text-center">
        ...
        <p className="text-muted-foreground mt-2 max-w-md text-sm">
          Translate PowerPoint presentations while preserving formatting using a
          local LLM. Supports multiple languages, speaker notes, and automatic
      ...
      <Button asChild className="bg-primary hover:bg-primary-light gap-2">
        <Link href="/samples/ppt-translator/demo">Open Full Demo</Link>
      </Button>
    </div>
  )
}
