...
try {
    Write-Host "Starting Diarization setup..." -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host ""
    Test-UV
    Write-Host "Diarization setup completed successfully!" -ForegroundColor Green
    exit 0
} catch {
    Write-Host "Diarization setup failed: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
} finally {
    Pop-Location
}
