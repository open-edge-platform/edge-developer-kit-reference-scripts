...
function Invoke-ChildSetup {
    ...
    if (Test-Path $SetupScript) {
        ...
        $script:SetupResults += @{ Name = $ChildName; Success = $null; Error = "No setup.ps1 found" }
    }
}
...
try {
    ...
    foreach ($ChildDir in $ChildDirectories) {
        try {
            ...
            Write-Host "Continuing with remaining text-to-speech workers..." -ForegroundColor Yellow
        }
    }
    ...
    if ($script:FailCount -gt 0) {
        Write-Host "Text-to-speech setup completed with errors." -ForegroundColor Red
        exit 1
    } else {
        Write-Host "All text-to-speech setup scripts completed successfully!" -ForegroundColor Green
        exit 0
    }
} catch {
    Write-Host "========================================" -ForegroundColor Red
    Write-Host "❌ FATAL ERROR in text-to-speech setup" -ForegroundColor Red
    Write-Host "========================================" -ForegroundColor Red
    Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Red
