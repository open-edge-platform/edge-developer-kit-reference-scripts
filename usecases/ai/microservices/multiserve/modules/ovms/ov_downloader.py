...
class OVDownloader:
    ...
    def __init__(self, models_base_dir: str = "models/OV", verified_model_file: str = VERIFIED_FILE_NAME):
        if os.name == 'nt':
    ...
    def read_verified_models(file_path: str) -> Dict[str, str]:
        try:
            ...
            verified_models = {
                repo_id: details 
                for repo_id, details in data['ov_models'].items() if 'task' in details
            }
    ...
    def get_model_info_for_repo(self, source_model: str) -> Dict:
        ...
        if model_path != "" or model_info:
            return { "task": found_task, "model_base_dir": self.models_base_dir, "model_path": model_path, "device": target_device_value, "tool_parser": model_info.get("tool_parser", ""), "chat_template": model_info.get("chat_template") }

        return {}
    
    def update_model_device(self, source_model: str, device: str):
        ...
        try:
            ...
            for line in process.stdout:
                ...
                if stripped_line.startswith("Downloading lfs size") or stripped_line.startswith("Progress:"):
                    print(stripped_line)
    ...
    def download_model(self, source_model: str) -> Generator[str, None, None]:
        ...
        try:
            ...
            if model_info.get("chat_template", "") != "":
                ...
                chat_template_path = Path(model_repository_path) / source_model / "chat_template.jinja"
            ...
            if self._current_process and self._current_process.returncode in (-signal.SIGTERM, -signal.SIGINT):
    ...
    def download_unverified_model(self, source_model: str, task: str) -> Generator[str, None, None]:
    ...
    def delete_downloaded_model(self, source_model: str) -> bool:
        ...
        if os.path.exists(model_dir):
            try:
                ...
                print(f"ERROR: Failed to delete model directory {model_dir}. Reason: {e}")
                return False
        else:
