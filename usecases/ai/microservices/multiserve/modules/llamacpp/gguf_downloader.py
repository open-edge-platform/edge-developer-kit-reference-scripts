...
class GGUFDownloader:
    ...
    KNOWN_QUANTS = { 
        "Q2_K", "Q2_K_L", "Q3_K_M", "Q3_K_S", "Q4_0", "Q4_1", 
        "Q4_K_M", "Q4_K_S", "Q5_K_M", "Q5_K_S", "Q6_K", "Q8_0", 
        "UD-Q8_K_L", "mxfp4", "f16"
    }
    
    def __init__(self, base_url: str = "https://huggingface.co/", models_base_dir: str = "models", verified_model_file: str = VERIFIED_FILE_NAME, token: Optional[str] = None):
    ...
    def _download_and_stream_progress(self, hf_repo: str, filename: str, local_path: str, tag: str = "main") -> Generator[Dict[str, float], None, str]:
                ...
                with open(temp_local_path, 'wb') as f:
                    for chunk in r.iter_content(chunk_size=chunk_size):
                        if self.cancellation_flag:
                            raise requests.exceptions.RequestException("Download cancelled by user.")
                        ...
                        if chunk:
                            ...
                            progress_pct = (downloaded_size / total_size) * 100 if total_size else 0
    ...
    def _calculate_sha256(self, file_path: str) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Checksum verification failed File not found at {file_path}")
    
    def _download_and_verify(self, manifest: Dict[str, str], task: str) -> List[Dict[str, str]]:
        self.cancellation_flag = False
        files_to_download = []
        ...
        for filename, expected_sha_key in files_to_check:
            ...
            local_path = str(Path(self.models_base_dir) / task / manifest['hf_repo'] / filename)
            expected_sha = manifest.get(expected_sha_key, '') if expected_sha_key else ''
    ...
    def read_verified_models(file_path: str) -> Dict[str, str]:
        try:
            ...
            verified_models = {
                repo_id: details 
                for repo_id, details in data['models'].items() if 'task' in details
            }

            return verified_models
        except FileNotFoundError:
            raise FileNotFoundError(f"Error Verified models file not found at {file_path}")
    ...
    def get_file_manifest(self, hf_repo_with_tag: str) -> Dict[str, str]:
        ...
        if cache_path.exists():
            try:
                ...
                print(f"Warning: Failed to load manifest from cache: {cache_e}. Attempting online retrieval.")
        ...
        try:
            ...
            if not gguf_file:
                raise RuntimeError("Error Model manifest does not contain a ggufFile entry")
            ...
            try:
                ...
                print(f"Status: Manifest successfully cached to {cache_path}", file=sys.stdout)
            except Exception as e:
                print(f"Warning: Failed to write manifest to cache: {e}", file=sys.stderr)

            return manifest_result

        except requests.exceptions.HTTPError as e:
            if e.response.status_code == 401:
                raise RuntimeError("Error 401 Unauthorized Did you provide a valid HF token")
            raise RuntimeError(f"HTTP Error {e.response.status_code} {e.response.text.strip()}")
        except Exception as e:
            raise RuntimeError(f"An unexpected error occurred during manifest retrieval: {e}")
    ...
    def list_all_cached_manifests(self) -> List[Dict[str, Any]]:
        ...
        for manifest_path in base_dir.rglob('*.json'):
            ...
            try:
                ...
                print(f"Warning: Skipping corrupted manifest file: {manifest_path}. Invalid JSON format.", file=sys.stderr)
            except Exception as e:
                print(f"Warning: Skipping manifest file {manifest_path} due to unexpected error: {e}", file=sys.stderr)
            ...
            for i, d_info in enumerate(files_to_download):
                ...
                if not expected_sha:
                    ...
                    if calculated_sha == expected_sha:
                        ...
                        raise RuntimeError(f"Verification FAILED for {filename}! Hashes do not match. File removed.")
    ...
    def delete_downloaded_model(self, hf_repo_with_tag: str) -> bool:
        ...
        try:
            ...
            for filename in files_to_delete:
                ...
                local_path = Path(self.models_base_dir) / task / manifest['hf_repo'] / filename
    ...
    def extract_quant_from_filename(self, gguf_filename: str) -> str:
        ...
        for known_quant in self.KNOWN_QUANTS:
            ...
            if (f".{check_string}.gguf" in filename_lower or 
    ...
    def list_verified_models(self) -> Dict[str, str]:
        ...
        for repo_id_with_tag, detail in self.verified_models.items():
            verified_models.append((repo_id_with_tag, detail['task'], detail['quant']))
    ...
    def list_downloaded_models(self) -> List[Tuple[str, str, List[str]]]:
        ...
        for manifest in cached_manifests:
            ...
            try:
                ...
                print(f"Warning: Skipping cached model {repo_id} due to file checking error: {e}", file=sys.stderr)
                continue
