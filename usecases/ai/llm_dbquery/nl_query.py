...
class SchemaDiscovery:
    ...
    def discover_all_schemas(cls, databases: Dict, force_refresh: bool = False, primary_db: str = None) -> Dict:
        ...
        if not primary_db_key:
            ...
            # 1. 'sales' - most common transactional database
            # 2. 'main' - common default name
            # 3. 'orders' - another common transactional database
            # 4. First database with most tables
            # 5. First database alphabetically
            
            priority_names = ['sales', 'main', 'orders', 'transactions']
...
"""
    def get_schema_description(self) -> str:
        ...
        if not self.schema_info:
            # Fallback to hardcoded schema if discovery failed
        ...
        for db_name, db_info in self.schema_info.items():
            prefix = db_info.get('prefix', '')
            schema_parts.append(f"\n{'PRIMARY' if not prefix else 'ATTACHED'} DATABASE ({db_name}):")
            if prefix:
                schema_parts.append(f"Prefix: {prefix}")
            ...
            for table_name, table_info in db_info.get('tables', {}).items():
                ...
                if table_info.get('sample_values'):
                    ...
                    for col_name, values in table_info['sample_values'].items():
                        if values:
                            schema_parts.append(f"  {col_name}: {', '.join(map(str, values[:5]))}")
        
        schema_parts.append("\n\nIMPORTANT RULES:")
        schema_parts.append("1. Use proper table prefixes for attached databases")
        schema_parts.append("2. DO NOT use database file names (no .sqlite in queries)")
        schema_parts.append("3. Use SQLite syntax (strftime for dates, ROUND for decimals)")
        schema_parts.append("4. **NEVER use SELECT * when joining** - Always specify columns with aliases")
...
2. Multiple queries are allowed, separate with semicolons
...
9. **NEVER use SELECT * when joining multiple tables** - Always specify columns explicitly with table aliases
10. When joining, use table aliases and prefix ALL columns (e.g., t1.column, t2.column)
11. Avoid ambiguous column names by always prefixing with table alias
...
class NaturalLanguageQueryInterface:
    ...
    def __init__(self, databases: Dict, primary_db: str = None):
        ...
        Args:
            databases: Dictionary of DatabaseConnection objects
            primary_db: Optional - specify which database should be primary
                       If None, uses intelligent auto-selection
        """
